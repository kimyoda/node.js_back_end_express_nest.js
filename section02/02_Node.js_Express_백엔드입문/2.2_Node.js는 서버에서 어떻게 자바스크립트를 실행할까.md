...
엔진은 파서, 컴파일러, 인터프리퍼, 가비지 컬렉터, 콜스택, 힙 으로 구성되어 있다.

> **가비지 콜렉터(grabage collector)**
> 메모리 누수를 방지하기 위해 주기적으로 사용하지 않는 메모리 공간을 회수하는 기능이다.

V8 엔진은 자바스크립트를 실행할 수 있는 엔진, 인터프리터 역할을 하는 이그니션과 컴파일러 역하을 하는 터보팬을 사용해 컴파일한다.

> **콜 스택(call stack)**
> 콜 스택은 현재 실행 중인 서브 루틴에 관한 정보를 저장하는 스택

> **힙(heap)**
> 힙은 객체나 동적 데이터가 저장되는 메모리 공간, 자료구조의 힙이 아니다.

![V8 엔진 자바스크립트 코드 컴파일 단계](./image.png)
